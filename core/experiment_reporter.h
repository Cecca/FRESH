...
MSGPACK_API_VERSION_NAMESPACE(MSGPACK_DEFAULT_API_NS) {
  ...
  template <> struct pack<element_value_t> {
    template <typename Stream>
    msgpack::packer<Stream> &operator()(msgpack::packer<Stream> &o,
                                        element_value_t const &v) const {
      switch (v.which()) {
      case 0: // size_t
        {
      ...
      case 5: // bool
        {
          if (boost::get<bool>(v)) o.pack_true();
          else o.pack_false();
          break;
        }
      }
  ...
  template <> struct pack<boost::posix_time::ptime> {
    template <typename Stream>
    msgpack::packer<Stream> &operator()(msgpack::packer<Stream> &o,
                                        boost::posix_time::ptime const &v) const {
...
#define EXPERIMENT_SET_RESULTS(results_ptr) \
  ExperimentReporter::get_instance().set_results(results_ptr)

#define EXPERIMENT_APPEND(table, ...)                           \
  ExperimentReporter::get_instance().append(table, __VA_ARGS__)

#define EXPERIMENT_TAG(key, val)                        \
  ExperimentReporter::get_instance().tag(std::string(key), val)

#define EXPERIMENT_SAVE()                       \
  ExperimentReporter::get_instance().save_msgpack()
