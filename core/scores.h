...
std::string baseline_path(const std::string basedir,
                          std::string queries_path,
                          std::string data_path,
                          double range) {
...
void dump_baseline(const std::set<std::pair<size_t, size_t>> &pairs,
...
Baseline load_baseline(const std::string basedir,
                       const std::string queries_path,
                       const std::string data_path,
                       double range) {
  std::string path = baseline_path(basedir, queries_path, data_path, range);
...
void print_false_positives(const std::set<std::pair<size_t, size_t>> &predictions,
  ...
  for (const auto &positive : predictions) {
    if (baseline.count(positive) == 0) {
      auto query = std::find_if(queries.begin(), queries.end(), [&](const Curve<Point> &c){ return c.id == positive.first; });
      auto curve = std::find_if(dataset.begin(), dataset.end(), [&](const Curve<Point> &c){ return c.id == positive.second; });
...
precision_recall_curve(const std::vector<std::pair<double, id_pair_t>> &scored_results,
  ...
  std::sort(pr_curve.begin(), pr_curve.end(),
            [&](double_pair_t a, double_pair_t b) {
              if (a.first < b.first) {
                return true;
...
std::vector<std::pair<double,double>>
roc(const std::vector<std::pair<double, id_pair_t>> &scored_results,
    const std::set<id_pair_t> &baseline,
    const size_t total_pairs) {
  // flags for the pairs
  std::vector<bool> flags(scored_results.size());
